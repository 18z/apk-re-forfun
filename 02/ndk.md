...
2018/06/01
修改 patch.py，使其可接參數
用 patch.py 改好 01 目錄下 binary files。
```
...
```
2018/06/19
用 vim 改好 08 目錄下 binary files。
得確認 x86, x86_64 是改一個地方還是改兩個地方。
此處只改有秀出完整 Hello from C++ 的部分。
```
...
```
2018/07/10
針對 08 目錄下的 arm64-v8a
用 r2 無法找到 he 或 He 字串。
用 vim 可找到
```

```
2019/04/02
一直無法 patch 成功，或許是因為 loader?
```
